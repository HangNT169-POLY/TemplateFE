...
public class MainServiceImpl implements MainService {
    ...
    private Student getStudentFromExcelQuiz(XSSFSheet worksheet, int index, String nameExcel) {
        Student student = new Student();
        XSSFRow row = worksheet.getRow(index);
        String name = removeAccent(row.getCell(1).toString()).substring(5).trim();
    ...
    private void readQuiz(Student student, XSSFRow row, String nameExcel) {
        if ("Quiz 1_results.xlsx".equalsIgnoreCase(nameExcel)) {
            student.setQuiz1(row.getCell(2).toString());
        }
        if ("Quiz 2_results.xlsx".equalsIgnoreCase(nameExcel)) {
            student.setQuiz2(row.getCell(2).toString());
        }
        if ("Quiz 3_results.xlsx".equalsIgnoreCase(nameExcel)) {
            student.setQuiz3(row.getCell(2).toString());
        }
        if ("Quiz 4_results.xlsx".equalsIgnoreCase(nameExcel)) {
            student.setQuiz4(row.getCell(2).toString());
        }
        if ("Quiz 5_results.xlsx".equalsIgnoreCase(nameExcel)) {
            student.setQuiz5(row.getCell(2).toString());
        }
        if ("Quiz 6_results.xlsx".equalsIgnoreCase(nameExcel)) {
            student.setQuiz6(row.getCell(2).toString());
        }
        if ("Quiz 7_results.xlsx".equalsIgnoreCase(nameExcel)) {
            student.setQuiz7(row.getCell(2).toString());
        }
        if ("Quiz 8_results.xlsx".equalsIgnoreCase(nameExcel)) {
